...
if($_GET["act"]=="search" && $_POST['title']!=''){
	 $where .= " and (title like '%".urlencode($_POST['title'])."%' or content like '%".$_POST['title']."%')";
}
if($_GET["userid"]){
...
<body>
    ...
    <tbody>
    <?php
	$page_sql = "select * from tbl_info where 1=1 and  daili='".$_SESSION['admin_user']."' $where";
	qy_page($page_sql,20);
	...
	<td><a href="../view.php?fid=<?=$row['infoid']?>" target="_blank"><?php echo urldecode($row['title']);?></a></td>
	<td><?=$row['userid']?></td>
