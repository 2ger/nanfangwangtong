...
// $sql="delete from tbl_ads_task where ".$where;
// mysql_query($sql);
// mysql_close();
...
if($_GET["act"]=="del"){
    ...
    mysql_query($sql);
    mysql_close();
    echo "<script type='text/javascript'>alert('成功删除!');location.href='adtasklist.php?page=".$_GET["page"]."';</script>";
    exit;
}
...
if($_GET['act']=='ischeck'){
    ...
    $row = mysql_fetch_array($result);
    $yi=$row['user_id'];
    $sql="update tbl_user set money=money+10 where id='{$yi}'";
    ...
    // exit($sql);
            ...
            if(trim($row['shuyu'])){
                ...
                if(!empty($row)){
                    ...
                    //写入交易日志
                    $sql="insert into tbl_acount_log (user_id,username,shouru,addtime,leixing) values ({$row['id']},'{$row['userid']}',3,{$time},'2级发圈奖励')";
                    mysql_query($sql);
                    ...
                    if(trim($row['shuyu'])){
                        ...
                        if(!empty($row)){
                            ...
                            mysql_query($sql);//次级关系会员加8
                            //写入交易日志
                            $sql="insert into tbl_acount_log (user_id,username,shouru,addtime,leixing) values ({$row['id']},'{$row['userid']}',3,{$time},'3级发圈奖励')";
                            mysql_query($sql);
                            $i=4;
                            // var_dump($row);exit;
                            do{
                                if(trim($row['shuyu'])){
                                    ...
                                    if(!empty($row)){
                                        ...
                                        if($row['groupid']==2){
                                            $er=$row['userid'];
                                            $sql="update tbl_user set money=money+0.3 where userid='{$er}'";
                                            mysql_query($sql);//次级关系会员加8
                                            //写入交易日志
                                            $sql="insert into tbl_acount_log (user_id,username,shouru,addtime,leixing) values ({$row['id']},'{$row['userid']}',0.3,{$time},'{$i}级发圈奖励，营销经理')";
                                            mysql_query($sql);
                                        }
                                    }
                                    $i++;
        ...
        function delall(){
            ...
            if(a>0){
                if(confirm('您已选中'+a+'条数据,是否确认删除?')){
                    document.forms.action='adtasklist.php?act=delall&page=<?=$_GET["page"]?>';
...
<div class="rightinfo">
    <div class="tools">
        <ul class="toolbar">
            ...
            <li>
                <a href="adtasklist.php?status=0">
                    <span>
		...
		<!---->
            <li>
                <a href="adtasklist.php?status=3">
                    <span>
            ...
            <li>
                <a href="adtasklist.php?status=2">
                    <span>
                       ...
                       echo "<font color='orange'>待上传</font>";
                        ...
                        //}
						      if(($row['ischeck']==0 || $row['ischeck']==2) && $row['ischeck']!=1){
                                echo "<a href='javascript:if(confirm(\"确定审核\")){location.href=\"?page={$_GET['page']}&act=ischeck&id={$row['id']}&status=1\";}'><font color='red'>未审核</font></a>";
                            }
							if($row['ischeck']==1){
...
<script type="text/javascript">
    ...
    $("img[id^='img_']").click(function(){
	   ...
	   $("b[id^='img_']").click(function(){
		...
		$.get('adtasklist.php?act=imagepath&tid='+tid,function(ret){
			console.log(ret);
