...
if(isset($_POST['regtime'])){
	...
	if(!$rowi){
		...
		$sql="UPDATE tbl_config SET regtime = '".$regtime."', regnum = '".$regnum."', regadnum = '".$regadnum."', reglognum = '".$reglognum."' , appversion = '".$appversion."' , downloadurl = '".$downloadurl."' , notice = '".$notice."' , content = '".$content."' WHERE id =".$rowi['id'];
		$res = mysql_query($sql);
...
<body>
	...
	<li><label>APP版本号</label><input name="appversion" type="text" class="dfinput" value="<?=$regconfig['appversion']?>" /><i>填整数字，如12，小于此的将会提示更新</i></li>
	<li><label>下载链接</label><input name="downloadurl" type="text" class="dfinput" value="<?=$regconfig['downloadurl']?>" /><i></i></li>
	<li><label>公告标题</label><input name="notice" type="text" class="dfinput" value="<?=$regconfig['notice']?>" /><i></i></li>
    <li><label>公告内容</label>
<textarea id="" name="content" cols="30" rows="10" style="width:500px;border:1px solid #ddd">
<?=$regconfig['content']?>
</textarea>
<i></i></li>
