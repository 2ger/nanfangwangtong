...
if($_GET['act']=='edit'){
	if($_POST["pwd"]){
		if($_POST["pwd"]==$_POST["repwd"]){
			...
			$pwdsql = "password = '".md5(md5($_POST['pwd'].'yitui'))."',";
		}else{
			echo "<script type='text/javascript'>alert('两次密码不一致!');history.go(-1);</script>";
			exit;
		}
...
<body>
    ...
    <ul class="forminfo">
    <li><label>用户登录名</label><input name="username" type="text" class="dfinput" value="<?=$row['username']?>" <?php if ($id){echo 'disabled';}?>/><i>唯一，不能重复</i></li>
